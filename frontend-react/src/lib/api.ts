...
export interface CRMOrganizationCreate {
    name: string;
    type?: string;
    email?: string;
    phone?: string;
...
export class ApiClient {
    ...
    async watchWorkItem(projectId: string, workItemId: string): Promise<ProjectWorkItem> {
        const response = await this.request<ProjectWorkItem>(`/projects/${projectId}/work-items/${workItemId}/watch`, {
            method: 'POST',
        });
        this.invalidateCacheByPrefix('/projects');
        return response;
    ...
    async unwatchWorkItem(projectId: string, workItemId: string): Promise<ProjectWorkItem> {
        const response = await this.request<ProjectWorkItem>(`/projects/${projectId}/work-items/${workItemId}/watch`, {
            method: 'DELETE',
        });
        this.invalidateCacheByPrefix('/projects');
    ...
    async moveWorkItem(projectId: string, workItemId: string, targetProjectId: string): Promise<ProjectWorkItem> {
        const response = await this.request<ProjectWorkItem>(`/projects/${projectId}/work-items/${workItemId}/move`, {
            method: 'POST',
    ...
    async getCRMStatus(): Promise<CRMStatus> {
        return this.cachedGet<CRMStatus>('/crm/status', SHORT_LIVED_CACHE_TTL_MS);
    }

    async getCRMDashboard(limit = 50): Promise<CRMDashboard> {
        return this.cachedGet<CRMDashboard>(`/crm/dashboard?limit=${limit}`, SHORT_LIVED_CACHE_TTL_MS);
    }

    async createCRMContact(data: CRMContactCreate): Promise<CRMContact> {
        const response = await this.request<CRMContact>('/crm/contacts', {
            method: 'POST',
            body: JSON.stringify(data),
        });
        this.invalidateCacheByPrefix('/crm');
        return response;
    }

    async getCRMContactDetail(contactId: string): Promise<CRMContactDetail> {
        return this.request<CRMContactDetail>(`/crm/contacts/${encodeURIComponent(contactId)}`);
    }

    async updateCRMContact(contactId: string, data: CRMContactUpdate): Promise<CRMContact> {
        const response = await this.request<CRMContact>(`/crm/contacts/${encodeURIComponent(contactId)}`, {
            method: 'PATCH',
            body: JSON.stringify(data),
        });
        this.invalidateCacheByPrefix('/crm');
    ...
    async addCRMContactNote(contactId: string, text: string): Promise<CRMNote> {
        return this.request<CRMNote>(`/crm/contacts/${encodeURIComponent(contactId)}/notes`, {
            method: 'POST',
            body: JSON.stringify({ text }),
        });
    }

    async createCRMTask(data: CRMTaskCreate): Promise<CRMTask> {
        const response = await this.request<CRMTask>('/crm/tasks', {
            method: 'POST',
            body: JSON.stringify(data),
        });
        this.invalidateCacheByPrefix('/crm');
    ...
    async updateCRMTask(taskId: string, data: { status?: string; done?: boolean }): Promise<CRMTask> {
        const response = await this.request<CRMTask>(`/crm/tasks/${encodeURIComponent(taskId)}`, {
            method: 'PATCH',
            body: JSON.stringify(data),
        });
        this.invalidateCacheByPrefix('/crm');
    ...
    async createCRMOrganization(data: CRMOrganizationCreate): Promise<CRMOrganization> {
        const response = await this.request<CRMOrganization>('/crm/organizations', {
            method: 'POST',
            body: JSON.stringify(data),
        });
        this.invalidateCacheByPrefix('/crm');
