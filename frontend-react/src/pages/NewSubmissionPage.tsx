...
export default function NewSubmissionPage() {
    ...
    const { user, isAuthenticated, isLoading: authLoading, refreshUser, canManageOperations } = useAuth();
    ...
    useEffect(() => {
        ...
        api.getProjects()
            .then((allProjects) => {
                const availableProjects = canManageOperations
                    ? allProjects
                ...
                if (!submissionId && availableProjects.length === 1) {
                    setSelectedProjectId(availableProjects[0].id);
                }
            })
            .catch(console.error);
    }, [isAuthenticated, submissionId, user, canManageOperations]);
    ...
    const handleAutoSaveForUpload = async (): Promise<string | null> => {
                            ...
                            {isEditing && (
                                <>
                                    ...
                                    <FileUpload
                                        ...
                                        disabledMessage="Select a project above before attaching files."
                                        onBeforeUpload={selectedProjectId && !submission?.id ? handleAutoSaveForUpload : undefined}
                                    />
                                </>
                            )}
                        </div>
