...
import { Button, EmptyState, Input, LoadingSpinner, Textarea, GuidancePanel } from '../components/ui';
import { Logo } from '../components/Logo';
import { useAuth } from '../lib/useAuth';
import { Search, Mail, Plus, Users, X, Filter, Edit2, Trash2, Upload, Image as ImageIcon, LayoutGrid, Grid2x2, List, ArrowRight, Layers, ChevronDown, ChevronRight } from 'lucide-react';
import {
...
type ProjectViewMode = 'grid' | 'compact' | 'list';
type ProjectSortKey = 'updated' | 'name' | 'status' | 'team';
...
const PROJECT_STATUS_GROUPS: { value: Project['status']; label: string }[] = [
...
export default function ProjectsPage() {
    ...
    const [viewMode, setViewMode] = useState<ProjectViewMode>(() => {
        const saved = typeof window !== 'undefined' ? window.localStorage.getItem('projectsViewMode') : null;
        return saved === 'list' || saved === 'compact' ? saved : 'grid';
    });
    const changeViewMode = (mode: ProjectViewMode) => {
        setViewMode(mode);
        try { window.localStorage.setItem('projectsViewMode', mode); } catch { /* ignore */ }
    };
    const [sortBy, setSortBy] = useState<ProjectSortKey>(() => {
        const saved = typeof window !== 'undefined' ? window.localStorage.getItem('projectsSortBy') : null;
        return saved === 'name' || saved === 'status' || saved === 'team' ? saved : 'updated';
    });
    const changeSortBy = (key: ProjectSortKey) => {
        setSortBy(key);
        try { window.localStorage.setItem('projectsSortBy', key); } catch { /* ignore */ }
    };
    const [groupByStatus, setGroupByStatus] = useState<boolean>(() => (
        typeof window !== 'undefined' && window.localStorage.getItem('projectsGroupByStatus') === 'true'
    ));
    const toggleGroupByStatus = () => {
        setGroupByStatus((value) => {
            const next = !value;
            try { window.localStorage.setItem('projectsGroupByStatus', String(next)); } catch { /* ignore */ }
            return next;
        });
    };
    const [collapsedGroups, setCollapsedGroups] = useState<Set<string>>(new Set());
    const toggleGroupCollapsed = (status: string) => {
    ...
    const sortedProjects = [...filteredProjects].sort((a, b) => {
        switch (sortBy) {
            ...
            case 'status':
                return (PROJECT_STATUS_ORDER[a.status] - PROJECT_STATUS_ORDER[b.status]) || a.name.localeCompare(b.name);
            case 'team':
                return (buildProjectTeamMembers(b).length - buildProjectTeamMembers(a).length) || a.name.localeCompare(b.name);
            case 'updated':
            default:
    ...
    const renderCompactCard = (project: Project) => (
        <ProjectCompactCard
            key={project.id}
            project={project}
            isAdmin={canManageOperations}
            ...
            <main id="main-content" className="main-content">
                <div className="container" style={{ padding: '0.5rem 1rem', maxWidth: '100%', margin: '0 auto' }}>
                    <div className="flex-between" style={{ marginBottom: '0.5rem' }}>
                        <div>
                            <h1 style={{ fontSize: '1.75rem', fontWeight: 700, margin: '0 0 0.25rem 0', color: 'var(--color-text-primary)', letterSpacing: '-0.02em' }}>Our Projects</h1>
                            <p style={{ color: 'var(--color-text-muted)', margin: 0, fontSize: '0.9rem' }}>Explore the initiatives our team is working on to make a difference.</p>
                        </div>
                        {canCreateProjects && (
                    ...
                    {/* Filters */}
                    <div className="card" style={{ display: 'flex', gap: '1rem', alignItems: 'center', marginBottom: '1rem', padding: '0.5rem 1rem', flexWrap: 'wrap' }}>
                        <div style={{ position: 'relative', flex: 1, minWidth: '250px' }}>
                            ...
                            <input
                                ...
                                style={{ width: '100%', padding: '0.35rem 1rem 0.35rem 2.2rem', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-md)', outline: 'none', fontSize: '0.85rem' }}
                                value={searchQuery}
                        ...
                        <div style={{ display: 'flex', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-md)', overflow: 'hidden', flexShrink: 0 }}>
                            {([
                                { mode: 'grid' as const, label: 'Grid view', icon: <LayoutGrid size={16} /> },
                                { mode: 'compact' as const, label: 'Compact grid', icon: <Grid2x2 size={16} /> },
                                { mode: 'list' as const, label: 'List view', icon: <List size={16} /> },
                            ]).map(({ mode, label, icon }) => (
                                <button
                                    key={mode}
                                    type="button"
                                    ...
                                    style={{ padding: '0.4rem 0.6rem', background: viewMode === mode ? 'var(--color-dark-bg)' : 'transparent', color: viewMode === mode ? 'var(--color-primary-gold)' : 'var(--color-text-secondary)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center' }}
                                >
                                    {icon}
                                </button>
                            ))}
                        ...
                        <select
                            value={sortBy}
                            onChange={(e) => changeSortBy(e.target.value as ProjectSortKey)}
                            aria-label="Sort projects"
                            title="Sort projects"
                            style={{ padding: '0.4rem 0.6rem', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-md)', fontSize: '0.8rem', background: 'white', cursor: 'pointer', color: 'var(--color-text-secondary)', flexShrink: 0 }}
                        >
                        ...
                        {viewMode === 'list' && (
                            <button
                                ...
                                style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', padding: '0.4rem 0.7rem', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-md)', fontSize: '0.8rem', fontWeight: 600, cursor: 'pointer', flexShrink: 0, background: groupByStatus ? 'var(--color-dark-bg)' : 'white', color: groupByStatus ? 'var(--color-primary-gold)' : 'var(--color-text-secondary)' }}
                            >
                    ...
                    {isLoading ? (
                        ...
                        viewMode === 'grid' ? (
                            ...
                            <div className="projects-grid">
                                {sortedProjects.map(renderCompactCard)}
                            </div>
                        ) : groupByStatus ? (
                            <div>
                                {PROJECT_STATUS_GROUPS.map((group) => {
                                    const groupProjects = sortedProjects.filter((p) => p.status === group.value);
                                    if (groupProjects.length === 0) return null;
                                    const collapsed = collapsedGroups.has(group.value);
                                    return (
                                        <div key={group.value} className="project-group">
                                            <div
                                                className="project-group-header"
                                                role="button"
                                                tabIndex={0}
                                                aria-expanded={!collapsed}
                                                onClick={() => toggleGroupCollapsed(group.value)}
                                                onKeyDown={(event) => {
                                                    if (event.key === 'Enter' || event.key === ' ') {
                                                        event.preventDefault();
                                                        toggleGroupCollapsed(group.value);
                                                    }
                                                }}
                                            >
...
function ProjectListRow({ project, isAdmin, onOpen, onEdit, onDelete }: ProjectCardProps) {
    const projectTags = normalizeProjectTags(project.tags);
    const teamMembers = buildProjectTeamMembers(project);
        ...
        >
            <div className="project-list-thumb">
                {project.banner_image ? (
                    ...
                    <Logo size={24} style={{ opacity: 0.45, filter: 'grayscale(100%)' }} />
                )}
            </div>

            <div className="project-list-main">
                <div className="project-list-titlerow">
            ...
            <div className="project-list-meta">
                <div className="avatar-stack hidden-mobile">
                    ...
                    {teamMembers.length > 4 && (
                        <div className="avatar-more" title={teamMembers.slice(4).map(getProjectUserDisplayName).join(', ')}>
                            +{teamMembers.length - 4}
                        </div>
                    )}
                </div>
                ...
                {isAdmin && (
                    <div className="project-list-actions">
                        <button onClick={(e) => { e.stopPropagation(); onEdit(); }} className="action-btn" title="Edit Project">
                            <Edit2 size={15} />
                        </button>
                        <button onClick={(e) => { e.stopPropagation(); onDelete(); }} className="action-btn delete" title="Delete Project">
                            <Trash2 size={15} />
                        </button>
                    </div>
                )}
                ...
                >
                    Open <ArrowRight size={15} style={{ marginLeft: '0.25rem' }} />
                </Button>
            </div>
        </div>
    );
...
function ProjectCompactCard({ project, isAdmin, onOpen, onEdit, onDelete }: ProjectCardProps) {
    const projectTags = normalizeProjectTags(project.tags);
    const teamMembers = buildProjectTeamMembers(project);
    ...
    return (
        <div
            ...
            style={{ cursor: 'pointer' }}
        >
            <div className="project-compact-top">
                <h3 className="project-compact-title">{project.name}</h3>
                <StatusBadge status={project.status} />
                {isAdmin && (
                    <div className="project-list-actions">
                        <button onClick={(e) => { e.stopPropagation(); onEdit(); }} className="action-btn" title="Edit Project"><Edit2 size={14} /></button>
                        <button onClick={(e) => { e.stopPropagation(); onDelete(); }} className="action-btn delete" title="Delete Project"><Trash2 size={14} /></button>
                    </div>
                )}
            </div>
            ...
            {projectTags.length > 0 && (
                <div className="project-compact-tags">
                    {projectTags.slice(0, 4).map((tag) => (<ProjectTagChip key={tag} tag={tag} />))}
                </div>
            )}

            <div className="project-compact-foot">
                <div className="avatar-stack">
                    {teamMembers.length > 0 ? (
                        teamMembers.slice(0, 5).map((member) => (
                            <div key={member.id} className="avatar-stack-item">
                        ...
                        <span style={{ fontSize: '0.78rem', color: '#94a3b8', fontStyle: 'italic' }}>No team</span>
                    )}
                    {teamMembers.length > 5 && (
                        <div className="avatar-more" title={teamMembers.slice(5).map(getProjectUserDisplayName).join(', ')}>+{teamMembers.length - 5}</div>
                    )}
                </div>
                <Button type="button" variant="ghost" size="sm" onClick={(event) => { event.stopPropagation(); onOpen(); }}>Open</Button>
            </div>
        </div>
    );
}
