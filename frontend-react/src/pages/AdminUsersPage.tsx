...
function InviteTableSection({ title, description, invites, onResendInvite, onRevokeInvite }: InviteTableSectionProps) {
    ...
    return (
        <div style={{ background: 'white', borderRadius: '16px', border: '1px solid #e2e8f0', boxShadow: '0 4px 15px rgba(0,0,0,0.02)', padding: '1.5rem', marginBottom: '1.25rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
                <div>
                    <h3 style={{ fontSize: '1.1rem', fontWeight: 800, margin: 0, color: '#0f172a' }}>{title} ({invites.length})</h3>
                    <p style={{ color: '#64748b', margin: '0.35rem 0 0', fontSize: '0.9rem', maxWidth: '720px', lineHeight: 1.5 }}>{description}</p>
                </div>
            </div>
                            ...
                            return (
                                <tr key={invite.id || `${invite.email}-${invite.created_at}`} style={{ borderBottom: '1px solid #f1f5f9' }} className="hover:bg-slate-50">
                                    <td style={{ padding: '1rem 1.5rem' }}>
                                        ...
                                        {invite.user_name && (
                                            <div style={{ marginTop: '0.35rem', color: '#64748b', fontSize: '0.82rem' }}>{invite.user_name}</div>
                                        )}
                                    </td>
                                    ...
                                    <td style={{ padding: '1rem' }}>
                                        <span style={{ display: 'inline-flex', alignItems: 'center', padding: '0.35rem 0.65rem', borderRadius: '999px', background: statusDisplay.background, color: statusDisplay.color, border: `1px solid ${statusDisplay.border}`, fontSize: '0.75rem', fontWeight: 700 }}>
                                            {statusDisplay.label}
                                        </span>
                                        {lastLoginLabel && (
                                            <div style={{ marginTop: '0.35rem', color: '#64748b', fontSize: '0.8rem' }}>Last login {lastLoginLabel}</div>
                                        )}
                                    </td>
                                    ...
                                    <td style={{ padding: '0.75rem 1.5rem', textAlign: 'right' }}>
                                        <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end' }}>
                                            <Button variant="ghost" size="sm" onClick={() => onResendInvite(invite.email)} style={{ fontSize: '0.8rem' }}>Resend</Button>
                                            <Button variant="ghost" size="sm" style={{ color: '#dc2626', fontSize: '0.8rem' }} onClick={() => onRevokeInvite(invite.email)}>Revoke</Button>
                                        </div>
                                    </td>
                                </tr>
                            );
                    ...
                    {canManageInvites && invites.length > 0 && (
                        <div style={{ marginBottom: '2rem' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', marginBottom: '1rem', flexWrap: 'wrap' }}>
                                <div>
                                    <h3 style={{ fontSize: '1.15rem', fontWeight: 800, margin: 0, color: '#0f172a' }}>Invite Records</h3>
                                    <p style={{ color: '#64748b', margin: '0.35rem 0 0', fontSize: '0.9rem', maxWidth: '760px', lineHeight: 1.5 }}>
