...
import { Users, Search, UserCheck, UserX, Shield, ChevronLeft, Plus, X, Mail, RefreshCw } from 'lucide-react';
...
const ADMIN_SCOPE_OPTIONS: Array<{ scope: AdminAccessScope; label: string; description: string }> = [
    ...
    { scope: 'manage_crm', label: 'Manage CRM', description: 'Open the CRM workspace with donor, partner, and contact relationship data.' },
...
function formatInviteRoleLabel(role: AllowedEmail['role']) {
    return role.replace('_', ' ').replace(/\b\w/g, (character) => character.toUpperCase());
}

function getInviteStatusDisplay(invite: AllowedEmail) {
    if (invite.portal_status === 'access_record') {
        if (invite.has_logged_in) {
            return {
                label: 'Logged in',
                background: '#ecfdf5',
                border: '#86efac',
...
function InviteTableSection({ title, description, invites, onResendInvite, onRevokeInvite }: InviteTableSectionProps) {
    if (invites.length === 0) {
        return null;
    }

    return (
        <div style={{ background: 'white', borderRadius: '16px', border: '1px solid #e2e8f0', boxShadow: '0 4px 15px rgba(0,0,0,0.02)', padding: '1rem', marginBottom: '1.25rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
                <div>
                    <h3 style={{ fontSize: '1.05rem', fontWeight: 800, margin: 0, color: '#0f172a' }}>{title} ({invites.length})</h3>
                    <p style={{ color: '#64748b', margin: '0.35rem 0 0', fontSize: '0.8rem', maxWidth: '720px', lineHeight: 1.5 }}>{description}</p>
                </div>
            </div>
            ...
            <div className="table-wrapper" style={{ margin: '0 -1.5rem -1.5rem', borderRadius: '0 0 16px 16px' }}>
                <table className="table" style={{ margin: 0 }}>
                    <thead style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                        <tr>
                            <th scope="col" style={{ padding: '0.25rem 0.5rem', color: '#64748b', fontWeight: 700, fontSize: '0.7rem', letterSpacing: '0.05em' }}>Email</th>
                            <th scope="col" style={{ padding: '0.25rem 0.5rem', color: '#64748b', fontWeight: 700, fontSize: '0.7rem', letterSpacing: '0.05em' }}>Role</th>
                            <th scope="col" style={{ padding: '0.25rem 0.5rem', color: '#64748b', fontWeight: 700, fontSize: '0.7rem', letterSpacing: '0.05em' }}>Portal Status</th>
                            <th scope="col" style={{ padding: '0.25rem 0.5rem', color: '#64748b', fontWeight: 700, fontSize: '0.7rem', letterSpacing: '0.05em' }}>Invited By</th>
                            <th scope="col" style={{ padding: '0.25rem 0.5rem', color: '#64748b', fontWeight: 700, fontSize: '0.7rem', letterSpacing: '0.05em' }}>Date</th>
                            <th scope="col" style={{ padding: '0.25rem 0.5rem', textAlign: 'right', color: '#64748b', fontWeight: 700, fontSize: '0.7rem', letterSpacing: '0.05em' }}>Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        {invites.map((invite) => {
                            const statusDisplay = getInviteStatusDisplay(invite);
                            const lastLoginLabel = invite.has_logged_in && invite.user_last_login
                                ? format(parseISO(invite.user_last_login), 'MMM d, yyyy')
                                : null;
                                ...
                                <tr key={invite.id || `${invite.email}-${invite.created_at}`} style={{ borderBottom: '1px solid #f1f5f9' }} className="hover:bg-slate-50">
                                    <td style={{ padding: '0.25rem 0.5rem' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#0f172a', fontWeight: 600 }}>
                                            <Mail size={14} style={{ color: '#94a3b8' }} /> <span style={{ fontSize: '0.8rem' }}>{invite.email}</span>
                                        </div>
                                        {invite.user_name && (
                                            <div style={{ marginTop: '0.1rem', color: '#64748b', fontSize: '0.75rem' }}>{invite.user_name}</div>
                                        )}
                                    </td>
                                    <td style={{ padding: '0.25rem 0.5rem' }}>
                                        <span style={{ display: 'inline-block', padding: '0.15rem 0.35rem', borderRadius: '4px', background: '#f1f5f9', color: '#475569', fontSize: '0.65rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                                            {formatInviteRoleLabel(invite.role)}
                                        </span>
                                    </td>
                                    <td style={{ padding: '0.25rem 0.5rem' }}>
                                        <span style={{ display: 'inline-flex', alignItems: 'center', padding: '0.15rem 0.35rem', borderRadius: '4px', background: statusDisplay.background, color: statusDisplay.color, border: `1px solid ${statusDisplay.border}`, fontSize: '0.65rem', fontWeight: 700 }}>
                                            {statusDisplay.label}
                                        </span>
                                        {lastLoginLabel && (
                                            <div style={{ marginTop: '0.1rem', color: '#64748b', fontSize: '0.7rem' }}>Last login {lastLoginLabel}</div>
                                        )}
                                    </td>
                                    <td style={{ padding: '0.25rem 0.5rem', color: '#64748b', fontSize: '0.8rem' }}>{invite.invited_by || 'System'}</td>
                                    <td style={{ padding: '0.25rem 0.5rem', color: '#64748b', fontSize: '0.8rem' }}>{format(parseISO(invite.created_at), 'MMM d, yyyy')}</td>
                                    <td style={{ padding: '0.25rem 0.5rem', textAlign: 'right' }}>
                                        <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end' }}>
                                            <Button variant="ghost" size="sm" onClick={() => onResendInvite(invite.email)} style={{ fontSize: '0.7rem', padding: '0.25rem 0.5rem' }}>Resend</Button>
                                            <Button variant="ghost" size="sm" style={{ color: '#dc2626', fontSize: '0.7rem', padding: '0.25rem 0.5rem' }} onClick={() => onRevokeInvite(invite.email)}>Revoke</Button>
                                        </div>
                                    </td>
                                </tr>
                            );
...
export default function AdminUsersPage() {
    ...
    const pendingLoginInvites = invites.filter((invite) => invite.portal_status !== 'access_record');
    const accessRecordInvites = invites.filter((invite) => invite.portal_status === 'access_record');
    ...
    return (
        <div className="page-wrapper" style={{ background: 'var(--color-bg-primary)', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
            ...
            <main id="main-content" className="main-content" style={{ flex: 1 }}>
                <div className="container" style={{ padding: '0.5rem 1rem', maxWidth: '100%', margin: '0 auto' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '0.5rem' }}>
                        <button 
                            onClick={() => navigate('/dashboard')} 
                            style={{ background: 'none', border: 'none', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', color: '#64748b', padding: 0 }}
                    ...
                    {canViewUsers && (
                    <div style={{ background: 'white', borderRadius: '16px', border: '1px solid #e2e8f0', boxShadow: '0 4px 15px rgba(0,0,0,0.02)', padding: '1rem', marginBottom: '1rem' }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem', marginBottom: '1rem' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                <h2 style={{ fontSize: '1.05rem', fontWeight: 800, margin: 0, color: '#0f172a', letterSpacing: '-0.01em' }}>All Users ({filteredUsers.length})</h2>
                            </div>
                                ...
                                <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                                    <div style={{ position: 'relative' }}>
                                        ...
                                        <input
                                            ...
                                            style={{ paddingLeft: '2rem', background: 'white', padding: '0.25rem 0.5rem 0.25rem 2rem', fontSize: '0.8rem', minHeight: '30px', width: '200px' }}
                                        />
                                    </div>
                                    <select id={roleFilterId} className="form-select" value={roleFilter} onChange={(e) => { setRoleFilter(e.target.value); setTimeout(loadUsers, 0); }} style={{ background: 'white', padding: '0.25rem 0.5rem', fontSize: '0.8rem', minHeight: '30px', width: 'auto' }}>
                                        <option value="">All Roles</option>
                                        <option value="volunteer">Volunteer</option>
                                        <option value="team_lead">Team Lead</option>
                                        <option value="admin">Admin</option>
                                    </select>
                                    <select id={statusFilterId} className="form-select" value={statusFilter} onChange={(e) => { setStatusFilter(e.target.value); setTimeout(loadUsers, 0); }} style={{ background: 'white', padding: '0.25rem 0.5rem', fontSize: '0.8rem', minHeight: '30px', width: 'auto' }}>
                                        <option value="">All Accounts</option>
                                        <option value="active">Enabled</option>
                                        <option value="inactive">Disabled</option>
                                    </select>
                                    {hasActiveFilters && (
                                        <Button variant="ghost" size="sm" onClick={() => { setSearchTerm(''); setRoleFilter(''); setStatusFilter(''); setTimeout(loadUsers, 0); }} style={{ padding: '0.25rem 0.5rem', fontSize: '0.8rem', minHeight: '30px' }}>
                                            Clear
                                        </Button>
                                    )}
                                </div>
                                <Button variant="ghost" size="sm" onClick={loadUsers} style={{ padding: '0.25rem 0.5rem', minHeight: '30px' }}>
                                    <RefreshCw size={14} />
                                </Button>
                            </div>
                        </div>
                        ...
                        {isLoading ? (
                            ...
                            <div className="table-wrapper" style={{ margin: '0 -1.5rem -1.5rem', borderRadius: '0 0 16px 16px' }}>
                                <table className="table" style={{ margin: 0 }}>
                                    <thead style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                                        <tr>
                                            <th scope="col" style={{ padding: '0.5rem 1rem', color: '#64748b', fontWeight: 700, fontSize: '0.7rem', letterSpacing: '0.05em' }}>User</th>
                                            <th scope="col" style={{ padding: '0.5rem 1rem', color: '#64748b', fontWeight: 700, fontSize: '0.7rem', letterSpacing: '0.05em' }}>Email</th>
                                            <th scope="col" style={{ padding: '0.5rem 1rem', color: '#64748b', fontWeight: 700, fontSize: '0.7rem', letterSpacing: '0.05em' }}>Role</th>
                                            <th scope="col" style={{ padding: '0.5rem 1rem', color: '#64748b', fontWeight: 700, fontSize: '0.7rem', letterSpacing: '0.05em' }}>Team</th>
                                            <th scope="col" style={{ padding: '0.5rem 1rem', color: '#64748b', fontWeight: 700, fontSize: '0.7rem', letterSpacing: '0.05em' }}>Stats</th>
                                            <th scope="col" style={{ padding: '0.5rem 1rem', color: '#64748b', fontWeight: 700, fontSize: '0.7rem', letterSpacing: '0.05em' }}>Last Login</th>
                                            <th scope="col" style={{ padding: '0.5rem 1rem', color: '#64748b', fontWeight: 700, fontSize: '0.7rem', letterSpacing: '0.05em' }}>Portal Status</th>
                                            <th scope="col" style={{ padding: '0.5rem 1rem', textAlign: 'right', color: '#64748b', fontWeight: 700, fontSize: '0.7rem', letterSpacing: '0.05em' }}>Actions</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {filteredUsers.map((user) => {
                                            ...
                                            <tr key={user.id} style={{ borderBottom: '1px solid #f1f5f9', transition: 'background 0.2s' }} className="hover:bg-slate-50">
                                                <td style={{ padding: '0.25rem 0.5rem' }}>
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                                        <div style={{ width: 24, height: 24, borderRadius: '50%', background: 'var(--color-primary-gold-light, #fef3c7)', color: 'var(--color-primary-gold)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: '0.7rem', border: '1px solid rgba(212, 175, 55, 0.2)', flexShrink: 0 }}>
                                                            {getUserInitials(user.name)}
                                                        </div>
                                                        <strong style={{ color: '#0f172a', fontSize: '0.8rem' }}>{user.name}</strong>
                                                    </div>
                                                </td>
                                                <td style={{ padding: '0.25rem 0.5rem', color: '#475569', fontSize: '0.8rem' }}>{user.email}</td>
                                                <td style={{ padding: '0.25rem 0.5rem' }}>
                                                    {canAdjustUserRole && roleEditorUser?.id === user.id ? (
                                                        <select className="form-select" value={roleEditorUser.role} onChange={(e) => handleRoleChange(user.id, e.target.value)} disabled={saving} style={{ minWidth: '120px', padding: '0.25rem 0.5rem', fontSize: '0.8rem' }}>
                                                            <option value="volunteer">Volunteer</option>
                                                            <option value="team_lead">Team Lead</option>
                                                ...
                                                <td style={{ padding: '0.25rem 0.5rem', color: '#64748b', fontSize: '0.8rem' }}>{user.team || '—'}</td>
                                                <td style={{ padding: '0.25rem 0.5rem', color: '#64748b', fontSize: '0.8rem' }}>
                                                ...
                                                <td style={{ padding: '0.25rem 0.5rem' }}>
                                                    {!user.is_active ? (
                                                        <span style={{ color: '#b91c1c', display: 'flex', alignItems: 'center', gap: '0.25rem', fontSize: '0.8rem', fontWeight: 600 }}>
                                                            <UserX size={14} /> Inactive
                                                        </span>
                                                    ) : hasLoggedIn ? (
                                                        <span style={{ color: '#15803d', display: 'flex', alignItems: 'center', gap: '0.25rem', fontSize: '0.8rem', fontWeight: 600 }}>
                                                            <UserCheck size={14} /> Active
                                                        </span>
                                                    ) : (
                                                        <span style={{ color: '#b45309', display: 'flex', alignItems: 'center', gap: '0.25rem', fontSize: '0.8rem', fontWeight: 600 }}>
                                                            <Mail size={14} /> Pending login
                                                        </span>
                                                    )}
                                                </td>
                                                <td style={{ padding: '0.25rem 0.5rem', textAlign: 'right' }}>
                                                        ...
                                                        <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end' }}>
                                                            ...
                                                            {canToggleUserStatus && (
                                                                <Button
                                                                    ...
                                                                    disabled={saving}
                                                                    style={{ color: user.is_active ? '#dc2626' : '#16a34a', fontSize: '0.7rem', padding: '0.35rem 0.75rem' }}
                                                                >
                    ...
                    {canManageInvites && invites.length > 0 && (
                        <div style={{ marginBottom: '1rem' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', marginBottom: '1rem', flexWrap: 'wrap' }}>
                                <div>
                                    <h3 style={{ fontSize: '1.05rem', fontWeight: 800, margin: 0, color: '#0f172a' }}>Invite Records</h3>
                                    <p style={{ color: '#64748b', margin: '0.35rem 0 0', fontSize: '0.8rem', maxWidth: '760px', lineHeight: 1.5 }}>
                                        Invite records keep an email approved for portal access. People can still appear here after login, so pending first login is shown separately below.
                                    </p>
                                </div>
                            ...
                            <InviteTableSection
                                ...
                                description="These people were invited and have not completed their first portal login yet."
                                invites={pendingLoginInvites}
                            ...
                            <InviteTableSection
                                ...
                                description="These invite records remain so the email stays approved for the portal, even after someone signs in."
                                invites={accessRecordInvites}
                                onResendInvite={handleResendInvite}
                                onRevokeInvite={handleRevokeInvite}
                            />
                        </div>
                    ...
                    {canManageInvites && invites.length === 0 && (
                        <div style={{ padding: '2rem', textAlign: 'center', background: 'white', borderRadius: '16px', border: '1px dashed #cbd5e1' }}>
                            <p style={{ color: '#64748b', margin: '0 0 1rem 0' }}>No invite records yet.</p>
                    ...
                    {canViewAdminAccess && (
                        <div style={{ background: 'white', borderRadius: '16px', border: '1px solid #e2e8f0', boxShadow: '0 4px 15px rgba(0,0,0,0.02)', padding: '1rem', marginTop: '2rem' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
                                <div>
                                    <h3 style={{ fontSize: '1.05rem', fontWeight: 800, margin: '0 0 0.25rem 0', color: '#0f172a' }}>Delegated Admin Access</h3>
                            ...
                            {grants.length > 0 ? (
                                <div className="table-wrapper" style={{ margin: '0 -1.5rem -1.5rem', borderRadius: '0 0 16px 16px' }}>
                                    <table className="table" style={{ margin: 0 }}>
                                        <thead style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                                            <tr>
                                                <th scope="col" style={{ padding: '0.25rem 0.5rem', color: '#64748b', fontWeight: 700, fontSize: '0.7rem', letterSpacing: '0.05em' }}>Delegate</th>
                                                <th scope="col" style={{ padding: '0.25rem 0.5rem', color: '#64748b', fontWeight: 700, fontSize: '0.7rem', letterSpacing: '0.05em' }}>Scopes</th>
                                                <th scope="col" style={{ padding: '0.25rem 0.5rem', color: '#64748b', fontWeight: 700, fontSize: '0.7rem', letterSpacing: '0.05em' }}>Granted By</th>
                                                <th scope="col" style={{ padding: '0.25rem 0.5rem', color: '#64748b', fontWeight: 700, fontSize: '0.7rem', letterSpacing: '0.05em' }}>Expires</th>
                                                <th scope="col" style={{ padding: '0.25rem 0.5rem', textAlign: 'right', color: '#64748b', fontWeight: 700, fontSize: '0.7rem', letterSpacing: '0.05em' }}>Actions</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {grants.map((grant) => (
                                                <tr key={grant.id} style={{ borderBottom: '1px solid #f1f5f9' }} className="hover:bg-slate-50">
                                                    <td style={{ padding: '0.25rem 0.5rem' }}>
                                                        <div style={{ fontWeight: 700, color: '#0f172a', fontSize: '0.8rem' }}>{grant.user_name}</div>
                                                        <div style={{ color: '#64748b', fontSize: '0.75rem' }}>{grant.user_email}</div>
                                                    </td>
                                                    <td style={{ padding: '0.25rem 0.5rem' }}>
                                                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.35rem' }}>
                                                            {grant.scopes.map((scope) => (
                                                                <span key={scope} style={{ display: 'inline-flex', alignItems: 'center', padding: '0.15rem 0.35rem', borderRadius: '4px', background: '#f8fafc', color: '#334155', fontSize: '0.65rem', fontWeight: 700, border: '1px solid #e2e8f0' }}>
                                                                    {formatAdminScopeLabel(scope)}
                                                                </span>
                                                            ))}
                                                        </div>
                                                        {grant.note && (
                                                            <div style={{ marginTop: '0.25rem', color: '#64748b', fontSize: '0.7rem' }}>{grant.note}</div>
                                                        )}
                                                    </td>
                                                    <td style={{ padding: '0.25rem 0.5rem', color: '#64748b', fontSize: '0.8rem' }}>{grant.granted_by_email}</td>
                                                    <td style={{ padding: '0.25rem 0.5rem', color: '#64748b', fontSize: '0.8rem' }}>
                                                        {grant.expires_at ? format(parseISO(grant.expires_at), 'MMM d, yyyy h:mm a') : 'No expiry'}
                                                    </td>
                                                    <td style={{ padding: '0.25rem 0.5rem', textAlign: 'right' }}>
                                                        {canManageAdminAccess ? (
                                                            <Button variant="ghost" size="sm" style={{ color: '#dc2626', fontSize: '0.7rem', padding: '0.25rem 0.5rem' }} onClick={() => handleRevokeGrant(grant.id)}>
                                                                Revoke
                                                            </Button>
                                                        ) : (
                                                            <span style={{ color: '#94a3b8', fontSize: '0.8rem' }}>View only</span>
            ...
            {canManageInvites && showInviteModal && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(15, 23, 42, 0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '0.5rem 1rem', animation: 'fadeIn 0.2s ease-out' }}>
                        ...
                        <form onSubmit={handleInviteUser}>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                                <div>
                                    ...
                                    <p style={{ fontSize: '0.7rem', color: '#64748b', marginBottom: '0.5rem' }}>Enter multiple emails separated by commas or new lines.</p>
                                    <textarea
                                        id={inviteEmailsId}
                                        required
                                        className="form-textarea"
                                        style={{ minHeight: '120px', resize: 'vertical', border: '1px solid #cbd5e1', borderRadius: '12px', padding: '0.5rem 1rem' }}
                    ...
                    <div role="dialog" aria-modal="true" aria-labelledby="edit-user-title" style={{ backgroundColor: 'white', borderRadius: '20px', padding: '2rem', width: '100%', maxWidth: '520px', boxShadow: '0 20px 40px rgba(0,0,0,0.2)', animation: 'slideUp 0.3s cubic-bezier(0.16, 1, 0.3, 1)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                            <div>
                                ...
                                <p style={{ margin: '0.45rem 0 0', color: '#64748b', fontSize: '0.8rem', lineHeight: 1.5 }}>
                        ...
                        <form onSubmit={handleSaveGrant} style={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', padding: '1.5rem 2rem', overflowY: 'auto', flex: 1, minHeight: 0 }}>
                                        ...
                                        {grantableScopeOptions.map((option) => (
                                            <label key={option.scope} style={{ display: 'flex', gap: '0.75rem', alignItems: 'flex-start', padding: '0.9rem', border: '1px solid #e2e8f0', borderRadius: '12px', cursor: 'pointer', minHeight: '100%' }}>
                                                ...
                                                <div>
                                                    ...
                                                    <div style={{ color: '#64748b', fontSize: '0.8rem' }}>{option.description}</div>
                                                </div>
                                            </label>
                                        ))}
                                    </div>
                                ...
                                <div>
                                    ...
                                    />
                                    <p style={{ fontSize: '0.7rem', color: '#64748b', marginTop: '0.4rem' }}>Leave empty if this delegated access should stay active until you revoke it.</p>
                                </div>
