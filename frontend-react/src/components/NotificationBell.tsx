...
export function NotificationBell() {
    ...
    const handleOpen = () => {
        const next = !open;
        setOpen(next);
        // Refresh on open so the list is current without waiting for the poll.
    ...
    const handleMarkAllRead = async () => {
        ...
        try {
            ...
            // Roll back to the server's truth if the call fails.
    ...
    const handleClick = (n: AppNotification) => {
        setOpen(false);
        // Mark just this one read (optimistically) before navigating.
        if (!n.read) {
            setNotifications(prev => prev.map(item => (item.id === n.id ? { ...item, read: true } : item)));
            setUnreadCount(prev => Math.max(0, prev - 1));
            api.markNotificationRead(n.id).catch(() => fetchNotifications());
        }
