...
This guide reflects the current portal behavior as of June 16, 2026.

## Recent Updates (June 2026)

**Project boards are now tabbed.** Each project page opens on a **Board** tab, with **Team**, **Files**, and (for project leads/managers) **Requests** tabs. The selected tab is saved in the URL, so links and refreshes keep their place. The header is more compact with an at-a-glance stats strip (team, work items, pending requests).

**Project leads can manage deletion requests.** The lead of a project can now approve or decline requests to delete that project, not just access/leadership requests. Approving permanently deletes the project and asks for confirmation first; the requester is notified if their request is declined.

**Projects directory has view options.** The Projects page now offers **Grid**, **Compact**, and **List** views, a **Sort** control (recently updated, name, status, team size), and **Group by status** (collapsible sections) in List view. Your preference is remembered.

**Notifications fixed and improved.** Clicking a notification (bell or email) now reliably opens the right page — "new request" notifications open the project's Requests tab directly. The bell marks a notification read when you open it and has a "Mark all read" action.

**New CRM workspace.** A CRM area (donor, partner, and contact relationships, backed by EspoCRM) is available to admins and anyone granted the new `manage_crm` delegated scope. The CRM nav link only appears if you have access.
