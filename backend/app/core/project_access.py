...
def can_edit_project_work_item(
    ...
    *,
    ...
    return is_project_team_member(project, user) and str(user.id) in assignee_ids
...
def can_delete_project_work_item(
    ...
    *,
    ...
    """Board managers can delete any work item; creators can delete their own."""
    if can_manage_project_work(project, user, operations_override=operations_override):
        return True

    created_by_id = getattr(work_item, "created_by_id", None)
    return is_project_team_member(project, user) and created_by_id is not None and str(created_by_id) == str(user.id)
