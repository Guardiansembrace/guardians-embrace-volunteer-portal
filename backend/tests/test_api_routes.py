...
def test_crm_dashboard_reports_setup_needed_when_espocrm_not_configured(api_client, monkeypatch):
    ...
    monkeypatch.setattr(
        crm_api,
        "EspoCRMClient",
        MagicMock(return_value=SimpleNamespace(is_configured=False)),
    )
    ...
    assert body["status"]["provider"] == "espocrm"
    assert body["contacts"] == []
    assert body["totals"]["relationships"] == 0


def test_crm_dashboard_maps_espocrm_records(api_client, monkeypatch):
    admin = make_user(role=UserRole.ADMIN)
    app.dependency_overrides[get_current_user] = lambda: admin

    fake_client = SimpleNamespace(is_configured=True)
    fake_client.list_records = AsyncMock(side_effect=[
        {
            "list": [
                {
                    "id": "contact-1",
                    ...
                    "emailAddress": "jordan@example.org",
                    "phoneNumber": "(555) 010-0001",
                    "accountId": "account-1",
                    "accountName": "Hope Center",
                    "assignedUserName": "Partnerships",
            ...
            "list": [
                {
                    ...
                    "dateEnd": "2026-05-27",
                    "assignedUserName": "Partnerships",
                    "parentName": "Hope Center",
                }
            ],
            "total": 1,
        },
    ])
    monkeypatch.setattr(crm_api, "EspoCRMClient", MagicMock(return_value=fake_client))
    ...
    assert body["contacts"][0]["name"] == "Jordan Lee"
    assert body["contacts"][0]["organization"] == "Hope Center"
    assert body["contacts"][0]["stage"] == "Active"
    assert body["organizations"][0]["name"] == "Hope Center"
    assert body["tasks"][0]["title"] == "Send roster"
    assert body["totals"] == {"relationships": 1, "follow_ups": 1, "partners": 1}


def test_crm_create_contact_creates_espocrm_record(api_client, monkeypatch):
    admin = make_user(role=UserRole.ADMIN)
    app.dependency_overrides[get_current_user] = lambda: admin
    fake_client = SimpleNamespace(is_configured=True)
    fake_client.create_record = AsyncMock(return_value={
        "id": "contact-9", "firstName": "Dana", "lastName": "Cruz",
        "emailAddress": "dana@example.org", "title": "Donor",
    })
    monkeypatch.setattr(crm_api, "EspoCRMClient", MagicMock(return_value=fake_client))

    response = api_client.post(
        "/api/v1/crm/contacts",
        json={"first_name": "Dana", "last_name": "Cruz", "email": "dana@example.org", "role": "Donor"},
    )

    assert response.status_code == 201
    body = response.json()
    assert body["name"] == "Dana Cruz"
    assert body["email"] == "dana@example.org"
    args, _ = fake_client.create_record.call_args
    assert args[0] == "Contact"
    assert args[1]["firstName"] == "Dana"


def test_crm_contact_detail_includes_activity(api_client, monkeypatch):
    admin = make_user(role=UserRole.ADMIN)
    app.dependency_overrides[get_current_user] = lambda: admin
    fake_client = SimpleNamespace(is_configured=True)
    fake_client.get_record = AsyncMock(return_value={
        "id": "contact-1", "firstName": "Jordan", "lastName": "Lee", "emailAddress": "j@example.org",
    })
    fake_client.list_stream = AsyncMock(return_value={"list": [{
        "id": "note-1", "type": "Post", "post": "Called about renewal",
        "createdByName": "Maxwell", "createdAt": "2026-06-16T10:00:00Z",
    }]})
    monkeypatch.setattr(crm_api, "EspoCRMClient", MagicMock(return_value=fake_client))

    response = api_client.get("/api/v1/crm/contacts/contact-1")

    assert response.status_code == 200
    body = response.json()
    assert body["contact"]["name"] == "Jordan Lee"
    assert body["activity"][0]["text"] == "Called about renewal"
    fake_client.get_record.assert_awaited_once_with("Contact", "contact-1")


def test_crm_add_contact_note_posts_to_stream(api_client, monkeypatch):
    admin = make_user(role=UserRole.ADMIN)
    app.dependency_overrides[get_current_user] = lambda: admin
    fake_client = SimpleNamespace(is_configured=True)
    fake_client.create_record = AsyncMock(return_value={
        "id": "note-2", "type": "Post", "post": "Sent thank-you",
        "createdByName": "Maxwell", "createdAt": "2026-06-16T12:00:00Z",
    })
    monkeypatch.setattr(crm_api, "EspoCRMClient", MagicMock(return_value=fake_client))

    response = api_client.post("/api/v1/crm/contacts/contact-1/notes", json={"text": "Sent thank-you"})

    assert response.status_code == 201
    assert response.json()["text"] == "Sent thank-you"
    args, _ = fake_client.create_record.call_args
    assert args[0] == "Note"
    assert args[1]["parentType"] == "Contact"
    assert args[1]["parentId"] == "contact-1"


def test_crm_update_task_marks_done(api_client, monkeypatch):
    admin = make_user(role=UserRole.ADMIN)
    app.dependency_overrides[get_current_user] = lambda: admin
    fake_client = SimpleNamespace(is_configured=True)
    fake_client.update_record = AsyncMock(return_value={"id": "task-1", "name": "Send roster", "status": "Completed"})
    monkeypatch.setattr(crm_api, "EspoCRMClient", MagicMock(return_value=fake_client))

    response = api_client.patch("/api/v1/crm/tasks/task-1", json={"done": True})

    assert response.status_code == 200
    assert response.json()["done"] is True
    fake_client.update_record.assert_awaited_once_with("Task", "task-1", {"status": "Completed"})


def test_crm_create_organization_creates_account(api_client, monkeypatch):
    admin = make_user(role=UserRole.ADMIN)
    app.dependency_overrides[get_current_user] = lambda: admin
    fake_client = SimpleNamespace(is_configured=True)
    fake_client.create_record = AsyncMock(return_value={"id": "acc-1", "name": "Hope Center", "type": "Partner"})
    monkeypatch.setattr(crm_api, "EspoCRMClient", MagicMock(return_value=fake_client))

    response = api_client.post("/api/v1/crm/organizations", json={"name": "Hope Center", "type": "Partner"})

    assert response.status_code == 201
    assert response.json()["name"] == "Hope Center"
    args, _ = fake_client.create_record.call_args
    assert args[0] == "Account"


def test_crm_write_requires_configuration(api_client, monkeypatch):
    admin = make_user(role=UserRole.ADMIN)
    app.dependency_overrides[get_current_user] = lambda: admin
    monkeypatch.setattr(crm_api, "EspoCRMClient", MagicMock(return_value=SimpleNamespace(is_configured=False)))

    response = api_client.post("/api/v1/crm/tasks", json={"title": "Follow up"})
...
def test_project_member_can_create_work_item_for_self(api_client, monkeypatch):
    ...
    assert created_docs[0].checklist[0].text == "Confirm desktop spacing"
...
def test_project_member_can_delete_work_item_they_created(api_client, monkeypatch):
    user = make_user(role=UserRole.VOLUNTEER)
    user.id = "65f0c10e8eced6afed0a8b21"
    ...
    fake_work_item = SimpleNamespace(
        id="work-item-created-by-user",
        project_id="project-delete-own",
        title="Remove my duplicate task",
    ...
    response = api_client.delete("/api/v1/projects/project-delete-own/work-items/work-item-created-by-user")

    assert response.status_code == 204
    fake_work_item.delete.assert_awaited_once()


def test_project_member_cannot_delete_someone_elses_work_item(api_client, monkeypatch):
    user = make_user(role=UserRole.VOLUNTEER)
    user.id = "65f0c10e8eced6afed0a8b22"
    user.name = "Project Member"
    app.dependency_overrides[get_current_user] = lambda: user
    ...
    fake_work_item = SimpleNamespace(
        id="work-item-created-by-other",
        project_id="project-delete-other",
        title="Someone else's task",
        created_by_id="65f0c10e8eced6afed0a8b23",
        delete=AsyncMock(),
    )
    ...
    response = api_client.delete("/api/v1/projects/project-delete-other/work-items/work-item-created-by-other")

    assert response.status_code == 403
    assert response.json()["detail"] == "Work item delete access required"
    fake_work_item.delete.assert_not_awaited()
...
def test_project_lead_can_approve_delete_request(api_client, monkeypatch):
